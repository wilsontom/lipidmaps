...
search_by_mass <- function(mz, adduct, ppm = 1.0)
  ...
  mz_res <- hlm_res <- httr::GET(query) %>% httr::content(., 'parsed')

  mz_res_clean<- mz_res %>% str_split('\n') %>% .[[1]]  %>% str_split('\t')
  ...
  mz_tib <- purrr::map(mz_res_clean2,
                  ~{tibble::tibble(field = mz_res_clean2[[1]],
                                                  value = .[1:9]) %>%
                      tidyr::spread(field,value)}) %>% bind_rows() %>% select(.,'Input m/z', 'Matched m/z', Delta, Name, Formula, Ion, Category, 'Main class', 'Sub class')
